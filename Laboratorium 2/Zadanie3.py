def Zadanie3(data):
    ...
    najdluzszy_napis = max(napis, key=len) if napis else None #funkcja max, pozwala na zwrócenie elementu, który znajduje się najwyżej alfabetycznie, lecz gdy zastosuje key=len, wtedy zostają porównywane długości
    
    krotka = list(filter(lambda x: isinstance(x, tuple), data))
    najdlusza_krotka = max(krotka, key=len) if krotka else None
